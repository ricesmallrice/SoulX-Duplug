...
class IndexTTS:
    def __init__(
        ...
        if audio_prompt is not None:
            try:
                ...
                self.auto_conditioning = auto_conditioning
            except:
                print("Reference wav loading failed.")

    def remove_long_silence(
        self, codes: list, latent: torch.Tensor, max_consecutive=15, silent_token=52
    ):
        assert (
            latent.dim() == 3 and latent.size(0) == 1
    ...
    async def infer(
        ...
        if self.auto_conditioning is None:
            auto_conditioning = []
            for ap_ in audio_prompt:
                audio, sr = torchaudio.load(ap_)
                audio = torch.mean(audio, dim=0, keepdim=True)
            ...
            auto_conditioning = self.auto_conditioning
